...
double j(double g, void * params)
{
	...
	double m=pp[4];
	double w0=pp[5];
	return(exp(-g)*cos(v*g+fasetm(t,d,g,thtm,m,w0)));
}
...
double k(double g, void * params)
{
	...
	double m=pp[4];
	double w0=pp[5];
	return(exp(-g)*sin(v*g+fasetm(t,d,g,thtm,m,w0)));/*Ai, ó: fasetm e fasear, de novo. Puta sacanagem*/
}
...
{  
   ...
   for(i=0;i<=n;i++)
	{
   	printf("%.10lf\t%.10lf\n",t*i/n,intensity(t*i/n,d,th,v,m,w0));
	}
   #pragma omp barrier
   return (0);
}
